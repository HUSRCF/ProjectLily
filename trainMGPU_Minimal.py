...
from torch.cuda.amp import autocast, GradScaler  # [ADDED] Only essential AMP import
...
class TrainingApp(tk.Tk):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        self.title("AudioSR Integrated Trainer - Enhanced (DataParallel + AMP + Smart torch.compile)")  # [MODIFIED] Updated title
        self.geometry("1000x900")  # [MODIFIED] Slightly larger window
    ...
    def create_widgets(self):
        ...
        ttk.Checkbutton(perf_frame, text="Mixed Precision (AMP)", variable=self.use_amp_var).pack(side='left', padx=10)

        # [MODIFIED] Changed from JIT to torch.compile
        self.jit_var = tk.BooleanVar(value=self.cfg['train'].get('use_torch_compile', True))
        ttk.Checkbutton(perf_frame, text="Smart torch.compile()", variable=self.jit_var).pack(side='left', padx=10)
    ...
    def run_training_logic(self):
        ...
        try:
            ...
            current_cfg['train']['use_torch_compile'] = self.jit_var.get()  # [MODIFIED] Updated name
            current_cfg['train']['use_mixed_precision'] = self.use_amp_var.get()  # [ADDED] AMP setting
            ...
            self.console.rule("[bold cyan]🚀 Starting Enhanced training (DataParallel + AMP + Smart torch.compile)[/bold cyan]")  # [MODIFIED] Updated message
            ...
            use_amp = current_cfg['train']['use_mixed_precision']  # [ADDED]
            use_compile = current_cfg['train']['use_torch_compile']  # [ADDED]
            ...
            self.console.print(f"Mixed Precision: [bold {'green' if use_amp else 'red'}]{use_amp}[/]")  # [ADDED]
            self.console.print(f"Smart torch.compile(): [bold {'green' if use_compile else 'red'}]{use_compile}[/]")  # [MODIFIED]
            ...
            if use_amp:
                scaler = GradScaler()
                self.console.print(f"[green]🔥 Mixed Precision enabled with GradScaler[/green]")
            ...
            # [MODIFIED] Wrap model with DataParallel first
            ...
            # [REVOLUTIONARY] Custom DataParallel + torch.compile() Integration
            if use_compile:
                try:
                    self.console.print("[cyan]Applying CUSTOM DataParallel + torch.compile() integration...[/cyan]")

                    # Strategy: Create a custom wrapper that compiles individual forward passes
                    class CompiledDataParallel(nn.DataParallel):
                        def __init__(self, module, device_ids=None, output_device=None, dim=0):
                            super().__init__(module, device_ids, output_device, dim)

                            # Compile the forward method AFTER DataParallel setup
                            self._compiled_forward = None
                            self._setup_compiled_forward()

                        def _setup_compiled_forward(self):
                            try:
                                # Create a compiled version of just the forward computation
                                original_module = self.module

                                def compiled_single_forward(batch):
                                    return original_module(batch)

                                # Compile the single-device forward function
                                self._compiled_forward = torch.compile(
                                    compiled_single_forward,
                        ...
                        def forward(self, *inputs, **kwargs):
                            # Use compiled forward for single GPU, regular DataParallel for multi-GPU
                            if len(self.device_ids) == 1 and self._compiled_forward is not None:
                                try:
                                    return self._compiled_forward(inputs[0])
                                except:
                                    # Fallback to regular forward
                                    pass

                            # Regular DataParallel forward for multi-GPU or if compilation failed
                            return super().forward(*inputs, **kwargs)

                    # Replace regular DataParallel with our custom version
                    if is_dataparallel:
                        # Get the unwrapped model first
                        original_model = model.module
                        # Create our custom DataParallel wrapper
                        model = CompiledDataParallel(original_model)
                        self.console.print("[green]✅ Custom CompiledDataParallel wrapper applied[/green]")
                        self.console.print("[cyan]💡 Uses torch.compile() for single-GPU ops, DataParallel for multi-GPU[/cyan]")
                    else:
                        # Single GPU: Apply torch.compile() normally
                        self.console.print("[cyan]Single GPU: Applying torch.compile() directly...[/cyan]")
                        model = torch.compile(
                            model,
                            mode='reduce-overhead',
                            fullgraph=False,
                        ...
                        self.console.print("[green]✅ Single GPU model compiled[/green]")

                except Exception as e:
                    self.console.print(f"[yellow]⚠️ Custom compilation failed: {str(e)}. Using standard DataParallel + AMP.[/yellow]")
            ...
            with Progress(
                ...
                for ep in range(current_cfg['train']['epochs']):
                    ...
                    for batch_idx, batch in enumerate(train_loader):
                        ...
                        # [ADDED] Mixed precision forward pass
                        if use_amp:
                            with autocast():
                                loss, _ = model(batch)
                        else:
                        ...
                        # [ADDED] Mixed precision backward pass
                        if use_amp:
                        ...
                        if (batch_idx + 1) % accumulation_steps == 0:
                            # [MODIFIED] Enhanced gradient clipping for mixed precision
                            if use_amp:
                                scaler.unscale_(opt)
                            ...
                            if gstep % current_cfg['train']['log_interval'] == 0:
                                # [ADDED] Enhanced logging with scale info for AMP
                            ...
                            if gstep % current_cfg['train']['valid_interval_steps'] == 0:
                                ...
                                for val_batch in valid_loader:
                                    ...
                                    with torch.no_grad():
                                        # [ADDED] Mixed precision validation
                                        if use_amp:
                                            with autocast():
