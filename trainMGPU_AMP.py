...
from torch.amp import autocast, GradScaler  # [ADDED] AMP support
...
class TrainingApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        # [ADDED] AMP support with BF16/FP16 optimization
        self.amp_var = tk.BooleanVar(value=self.cfg['train'].get('use_mixed_precision', True))
        ttk.Checkbutton(perf_frame, text="Mixed Precision (BF16/FP16)", variable=self.amp_var).pack(side='left', padx=10)
    ...
    def run_training_logic(self):
        ...
        try:
            ...
            # [ADDED] AMP config
            current_cfg['train']['use_mixed_precision'] = self.amp_var.get()
            ...
            epsilon = 1e-8
            # [FIX] Use non-inplace operations to avoid gradient computation errors
            with torch.no_grad():
            ...
            if torch.cuda.device_count() > 1:
                ...
                # [FIX] Ensure model is in training mode before wrapping with DataParallel
                model.train()
            ...
            # [ADDED] AMP scaler initialization with BF16 optimization
            use_amp = current_cfg['train'].get('use_mixed_precision', False)
            scaler = None
            amp_dtype = torch.float16  # Default dtype

            if use_amp:
                try:
                    # [SOLUTION 1] Enable optimized AMP settings for better compatibility
                    scaler = GradScaler(device='cuda',
                                      init_scale=2.**16,    # Higher initial scale
                                      growth_factor=2.0,    # Conservative growth
                                      backoff_factor=0.5,   # Conservative backoff
                                      growth_interval=2000) # Less frequent scale updates

                    # [SOLUTION 2] Use BF16 for better performance and numerical stability
                    # BF16 has better range than FP16 and is more stable for training

                    # Check if BF16 is supported on this hardware
                    if torch.cuda.is_bf16_supported():
                        amp_dtype = torch.bfloat16
                        self.console.print("[cyan]✓ Using BF16 (Brain Float 16) for optimal performance[/cyan]")
                        self.console.print("[cyan]  • Better numerical range than FP16[/cyan]")
                        self.console.print("[cyan]  • More stable training[/cyan]")
                    else:
                        amp_dtype = torch.float16
                        # Fallback to TF32 optimizations for FP16
                        torch.backends.cudnn.allow_tf32 = True
                        torch.backends.cuda.matmul.allow_tf32 = True
                        self.console.print("[cyan]✓ Using FP16 with TF32 optimizations[/cyan]")
                        self.console.print("[cyan]  • Hardware doesn't support BF16[/cyan]")

                    self.console.print("[cyan]✓ Mixed Precision (AMP) enabled with performance optimizations[/cyan]")
                    self.console.print("[cyan]  • Conservative gradient scaling[/cyan]")

                except Exception as e:
                    use_amp = False
                    scaler = None
                    self.console.print(f"[red]✗ AMP initialization failed: {e}[/red]")
                    self.console.print("[cyan]Falling back to FP32 training[/cyan]")
            else:
                self.console.print("[cyan]Using FP32 training[/cyan]")
            ...
            with Progress(
                ...
                # [FIX] Remove overall progress bar, only keep epoch progress
                ...
                for ep in range(current_cfg['train']['epochs']):
                    ...
                    # [FIX] Improved progress display with proper initialization
                    ...
                    current_epoch_task = progress.add_task(
                        f"[cyan]Epoch {ep+1}/{current_cfg['train']['epochs']}",
                        total=stable_batch_count,
                        loss=loss_display,
                        lr=lr_display
                    )
                    opt.zero_grad()
                    ...
                    for batch_idx, batch in enumerate(train_loader):
                        ...
                        # [MODIFIED] Forward pass with AMP support and optimal dtype
                        if use_amp:
                            with autocast(device_type='cuda',
                                        dtype=amp_dtype,        # BF16 or FP16 based on hardware
                        ...
                        # [MODIFIED] Backward pass with AMP support and error handling
                        try:
                            if use_amp:
                            ...
                            if "modified by an inplace operation" in str(e):
                                self.console.print(f"[red]⚠️ In-place operation error detected at step {gstep}. Skipping this step.[/red]")
                                self.console.print(f"[dim]Error details: {str(e)[:100]}...[/dim]")
                        ...
                        if current_epoch_task is not None:
                            try:
                                ...
                                # Silent fail for progress updates to avoid spam
                                pass
...
# =====================================================================================
# SECTION: NON-GUI TRAINING LOGIC
# =====================================================================================
...
def run_training_nogui(config):
    ...
    try:
        ...
        valid_set = PairedMelDataset(split='valid', **config['data'], preload_to_ram=True)

        train_buffer = []
        loader_thread = None

        if config['train']['fastboot']:
            # For non-GUI mode, we can simplify this and just load all data
            console.print("Loading full training data...")
            for i in range(len(train_set_full)):
                item = train_set_full._load_item(i)
                if item: train_buffer.append(item)
            console.print(f"[green]Full dataset loaded ({len(train_buffer)} items).[/green]")
            train_loader = DataLoader(train_buffer, batch_size=config['train']['batch_size'], shuffle=True)
        else:
            console.print("Loading full training data synchronously...")
            for i in range(len(train_set_full)):
                item = train_set_full._load_item(i)
                if item: train_buffer.append(item)
            console.print("[green]Full dataset loaded.[/green]")
            train_loader = DataLoader(train_buffer, batch_size=config['train']['batch_size'], shuffle=True)

        valid_loader = DataLoader(valid_set, batch_size=config['train']['batch_size'], shuffle=False)

        # --- 3. Model, Optimizer, Scheduler, EMA ---
        model_config = config['model'].copy()

        # Set gradient checkpointing based on config
        ...
        if config['train'].get('pretrained_path'):
            load_pretrained(model, config['train']['pretrained_path'], device, console)

        # Wrap model with DataParallel if multiple GPUs are available
        is_dataparallel = False
        if torch.cuda.device_count() > 1:
            console.print(f"[bold green]✅ Using {torch.cuda.device_count()} GPUs for training via DataParallel.[/bold green]")
            # [FIX] Ensure model is in training mode before wrapping with DataParallel
            model.train()
            model = nn.DataParallel(model)
            is_dataparallel = True

        # Get the original model from the wrapper if it exists
        unwrapped_model = model.module if is_dataparallel else model

        # For non-GUI mode, use full model training by default
        # This can be extended later to support config-based layer selection
        trainable_params = list(unwrapped_model.parameters())

        # Check JIT compatibility with DataParallel
        use_jit = config['train']['use_jit_compile']
        if use_jit and is_dataparallel:
            console.print("[yellow]⚠️ Skipping JIT compilation: Incompatible with nn.DataParallel.[/yellow]")
            use_jit = False

        if use_jit:
            if not use_grad_checkpoint:
                console.print("[cyan]Compiling model with Torch JIT...[/]")
                model = torch.jit.script(model)
                console.print("[green]Model compiled.[/green]")
            else:
                console.print("[yellow]Skipping JIT: Incompatible with Gradient Checkpointing.[/yellow]")

        console.print("[cyan]Training mode: full model[/]")

        opt = torch.optim.AdamW(trainable_params, lr=config['model']['params']['base_learning_rate'], betas=tuple(config['train']['betas']), weight_decay=config['train']['weight_decay'])

        # AMP scaler initialization with BF16 optimization
        use_amp = config['train'].get('use_mixed_precision', False)
        scaler = None
        amp_dtype = torch.float16  # Default dtype
        ...
        if use_amp:
            try:
                ...
                if torch.cuda.is_bf16_supported():
                    amp_dtype = torch.bfloat16
                    console.print("[cyan]✓ Using BF16 (Brain Float 16) for optimal performance[/cyan]")
                    console.print("[cyan]  • Better numerical range than FP16[/cyan]")
                    console.print("[cyan]  • More stable training[/cyan]")
                else:
                    amp_dtype = torch.float16
                    # Fallback to TF32 optimizations for FP16
                    torch.backends.cudnn.allow_tf32 = True
                    torch.backends.cuda.matmul.allow_tf32 = True
                    console.print("[cyan]✓ Using FP16 with TF32 optimizations[/cyan]")
                    console.print("[cyan]  • Hardware doesn't support BF16[/cyan]")

                console.print("[cyan]✓ Mixed Precision (AMP) enabled with performance optimizations[/cyan]")
                console.print("[cyan]  • Conservative gradient scaling[/cyan]")

            except Exception as e:
                use_amp = False
                scaler = None
                console.print(f"[red]✗ AMP initialization failed: {e}[/red]")
                console.print("[cyan]Falling back to FP32 training[/cyan]")
        else:
            console.print("[cyan]Using FP32 training[/cyan]")

        num_batches = len(train_set_full)
        max_steps = (config['train']['epochs'] * num_batches) // accumulation_steps

        sched = WarmupCosine(opt, config['model']['params']['base_learning_rate'], warmup=config['train']['warmup_steps'], max_steps=max_steps)

        # Initialize EMA with the unwrapped model
        ema = EMA(unwrapped_model, decay=config['train']['ema_decay'])

        # Initialize advanced loss monitor
        loss_monitor = None
        if config['train'].get('loss_explosion_protection', False) or config['train'].get('adaptive_lr', False):
            loss_monitor = AdvancedLossMonitor(
                explosion_threshold=config['train'].get('explosion_threshold', 10.0),
                lr_reduction_factor=config['train'].get('lr_reduction_factor', 0.5),
                patience=config['train'].get('patience', 1000) if config['train'].get('early_stopping', False) else float('inf')
            )
            console.print(f"[cyan]Advanced Loss Monitor enabled:[/cyan]")
            console.print(f"[dim]  Explosion threshold: {loss_monitor.explosion_threshold}[/dim]")
            console.print(f"[dim]  LR reduction factor: {loss_monitor.lr_reduction_factor}[/dim]")
            if config['train'].get('early_stopping', False):
                console.print(f"[dim]  Early stopping patience: {loss_monitor.patience} steps[/dim]")
        ...
        current_loss_value = 0.0
        current_lr_value = config['model']['params']['base_learning_rate']
        ...
        with Progress(
            ...
            for ep in range(config['train']['epochs']):
                ...
                if loss_monitor is not None:
                    stats = loss_monitor.get_stats()
                    if stats['steps_without_improvement'] >= loss_monitor.patience and config['train'].get('early_stopping', False):
                        console.print(f"[red]Early stopping: training terminated at epoch {ep+1}[/red]")
                        early_stop_triggered = True
                        break
                ...
                stable_batch_count = len(train_loader)
                current_epoch_task = progress.add_task(
                    f"[cyan]Epoch {ep+1}/{config['train']['epochs']}",
                    total=stable_batch_count,
                    loss=loss_display,
                    lr=lr_display
                )
                ...
                for batch_idx, batch in enumerate(train_loader):
                    ...
                    # Forward pass with AMP support and optimal dtype
                    if use_amp:
                        with autocast(device_type='cuda',
                                    dtype=amp_dtype,        # BF16 or FP16 based on hardware
                                    enabled=True):          # Explicit enable
                    ...
                    # Handle loss gathering from multiple GPUs by taking the mean
                    if is_dataparallel:
                        loss = loss.mean()
                    ...
                    if loss_monitor is not None:
                        ...
                        if actions['message']:
                            console.print(f"[yellow]{actions['message']}[/yellow]")

                        if actions['skip_step']:
                            skip_this_step = True
                            console.print(f"[red]Skipping step {gstep} due to NaN/Inf loss[/red]")

                        if actions['reduce_lr'] and config['train'].get('adaptive_lr', False):
                            new_lr = loss_monitor.reduce_learning_rate(opt, console)
                            current_lr_value = new_lr

                        if actions['stop_training'] and config['train'].get('early_stopping', False):
                            console.print(f"[red]Early stopping triggered at step {gstep}[/red]")
                            early_stop_triggered = True
                            break
                    ...
                    # Backward pass with AMP support and error handling
                    try:
                        if use_amp:
                            scaler.scale(loss).backward()
                        else:
                        ...
                        if "modified by an inplace operation" in str(e):
                            console.print(f"[red]⚠️ In-place operation error detected at step {gstep}. Skipping this step.[/red]")
                            console.print(f"[dim]Error details: {str(e)[:100]}...[/dim]")
                            opt.zero_grad()  # Clear gradients
                            continue
                        else:
                            raise e
                    ...
                    if (batch_idx + 1) % accumulation_steps == 0:
                        # Enhanced gradient clipping and optimizer step
                        if use_amp:
                            scaler.unscale_(opt)
                            trainable_params_for_clipping = [p for p in unwrapped_model.parameters() if p.requires_grad and p.grad is not None]
                            if trainable_params_for_clipping:
                        ...
                        if gstep % config['train']['log_interval'] == 0:
                            ...
                            if loss_monitor is not None and gstep % (config['train']['log_interval'] * 4) == 0:
                                stats = loss_monitor.get_stats()
                                console.print(f"[dim]📊 Monitor Stats - Best: {stats['best_loss']:.4f}, "
                                             f"No improvement: {stats['steps_without_improvement']}, "
                                             f"LR reductions: {stats['lr_reductions']}, "
                                             f"Short MA: {stats['short_ma']:.4f}, Long MA: {stats['long_ma']:.4f}[/dim]")

                        if gstep % config['train']['valid_interval_steps'] == 0:
                            model.eval()
                            val_losses = []
                            for val_batch in valid_loader:
                                val_batch = {k: v.to(device) for k, v in val_batch.items() if isinstance(v, torch.Tensor)}
                                with torch.no_grad():
                                    val_loss, _ = model(val_batch)
                                # Average validation loss across GPUs
                                if is_dataparallel:
                                    val_loss = val_loss.mean()
                                val_losses.append(val_loss.item())
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='AudioSR Training with DataParallel + AMP Support')
    parser.add_argument('--term', action='store_true', help='Run training in terminal mode without GUI')
    ...
    try:
        ...
        print(f"错误: 配置文件 '{args.config}' 未找到。请确保它在脚本所在的目录中。")
        sys.exit(1)
    except Exception as e:
        print(f"启动训练时发生严重错误: {e}")
        sys.exit(1)
